...
+ =threads per core= :: The number of threads per core (hyperthreading).
...
+ =serial job launcher= :: Specify the serial job launcher (if there is one).
...
This configuration file describes which batch system to use with the 'bolt' job submission script generation tool. 

The configuration files for all batch systems reside in:
#+BEGIN_SRC bash
$BOLT_DIR/configuration/batch
#+END_SRC

and must have the extension '.batch'.

The PBSPro.batch file has been extensively annotated to assist in setting up your own configuration. All options must exist in the file even if they do not have a etting.

A full list of options is now presented organised by sections.

*** [system info]
This section defines the name of the batch system.

+ =name= :: Name of the batch system.
+ =submit command= :: Name of the command to submit the job.
*** [basic options]
This section includes options that apply to both parallel or serial jobs.

+ =option identifier= :: The string that precedes every batch option specified in the job submission script.
+ =job name option= :: Specifies the job name.
+ =account option= :: Specifies the account to charge the job to.
+ =queue option= :: specifies the queue name.


*** [parallel options]
This section defines all otpions specific to parallel jobs.

+ =parallel option= :: Specifies the number of parallel tasks. 
+ =task per node option= :: Specifies number of tasks per node.
+ =taks per die option= :: Specifies the number of tasks per die.
+ =task stride option= :: Specifies the preferresd stride between parallel tasks.
+ =time option= :: Specifies the parallel walltime.
+ =additional options= :: Any additional options to add for parallel jobs. Note these should be specified without the option identifier as this will be added automatically. Multiple options should be separated by a ',' (comma).
+ =script preamble= :: Any script lines to include in parallel jobs before the application is launched.
+ =script postamble= ::Any script lines to include in parallel jobs after the application has completed.



*** [serial options]
All options specific to a serial job.

+ =time option= :: Specifies serial walltime.
+ =additional options= ::Any additional options to add for serial jobs.
+ =script preamble= :: Any script lines to include in serial jobs before the application is launched.
+ =script postamble= ::Any script lines to include in serial jobs after the application has completed.

** Global configuration file

The global configuration file sets the global options for the bolt tool. Currently only the default resource to use is available. This resource name must match one of the resources defines in /configuration/resources/*.resource.
If the user wants to use a different resource, either the entry in this file can be changed or 'bolt -r <OtherSource> ' has to be run.
*** [global options]

+ =default resource= :: Default resource to be used.

** Codes

A code configuration file describes a specific code, e.g. cp2k on HECToR. 

*** [code info]
+ =name= :: Name of the code, as to be used for selecting it.
+ =description= :: Description of the code as to be displayed for information.
+ =runtime message= ::Message to be printed when bolt generates a script for this code.

*** [job settings]
This sections defines the various available job types. Of an entry is blank, this type of job is not available.

+ =parallel= :: Distributed-memory/MPI version of code.
+ =serial= :: Serial version of code.
+ =hybrid= :: Hybrid (OpenMP/MPI) version of code.
+ =number of arguments= :: Number of command line arguments required.
+ =argument format= :: Format string for command line arguments. Enclose argument indices in braces and they will be replaced by actual argument values when script is produced.

*** [job limits]
This section handles any limits on the number of cores that can be used for parallel jobs. If zero then ???

+ =maximum tasks= :: Maximum number of parallel tasks.
+ =minimum tasks= :: Minimum number of parallel tasks.

*** [scipt commands]
This section covers any additional commands to be added to the script.

+ =preamble= :: Commands to run before the job is launched.
+ =postamble= :: Commands to run after the job has run.
